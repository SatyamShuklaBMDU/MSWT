...
$(document).ready(function() {
    ...
    //         url    : registerUrl, // check this route in web.php
    //         type   : 'POST',
    ...
    $.ajax({
        type: 'POST',
        url: '/user/register', // Use the named route
        data: formData,
        success: function(resp) {
            if (resp.type == 'error') {
                    $('.loader').hide();
    ...
    var formData = new FormData(this); // FormData object create karna

    var fileInput = document.getElementById('fileInput');
    var file = fileInput.files[0];

    // FormData object me file ko add karna
    ...
    $.ajax({
        ...
        type   : 'POST',
        data   : formData, // Sending FormData object to server with the AJAX request
        contentType: false, // FormData ko process nahi karna hai
        processData: false, // FormData ka content type set karna hai
        success: function(resp) {
            ...
            if (resp.type == 'error') {
                ...
                $.each(resp.errors, function(i, error) {
                    $('#account-' + i).css('color', 'red').html(error);
                    setTimeout(function() {
                        $('#account-' + i).hide();
                    }, 3000);
                });
                ...
                $('#account-success').css('color', 'green').html(resp.message);
                setTimeout(function() {
        ...
        error  : function() {
            // Hide loader and show error message if AJAX request is unsuccessful
            $('.loader').hide();
            alert('Error');
        }
    });
