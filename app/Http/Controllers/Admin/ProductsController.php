...
//dd($products);
    ...
    { // If the $id is not passed, this means 'Add a Product', if not, this means 'Edit the Product'
        ...
        if ($Did == '') { // if there's no $id is passed in the route/URL parameters, this means 'Add a new product'
            $title = 'Add Product';
        ...
        if ($request->isMethod('post')) { // WHETHER 'Add a Product' or 'Update a Product' <form> is submitted (THE SAME <form>)!!
            ...
            if ($request->hasFile('product_image')) {
                ...
                if ($image_tmp->isValid()) {
                    ...
                    $image_tmp->move(public_path('front/images/product_images'), $imageName);
            ...
            if ($Did == '') { // if a NEW product is added by an 'admin' or 'vendor', assign those new values. Otherwise, when Edit/Update an already existing product, leave everything as is
    ...
    { // Add/Edit Attributes function
        Session::put('page', 'products');
        $Did = $id ? decrypt($id) : '';
        $product = Product::select('id', 'product_name', 'product_code', 'product_color', 'product_price', 'product_image')->with('attributes')->find($Did); // with('attributes') is the relationship method name in the Product.php model
        ...
        if ($request->isMethod('post')) { // When the <form> is submitted
            ...
            foreach ($data['sku'] as $key => $value) { // or instead could be: $data['size'], $data['price'] or $data['stock']
                ...
                if (!empty($value)) {
                    ...
                    $sizeCount = ProductsAttribute::where(['product_id' => $Did, 'size' => $data['size'][$key]])->count();
                    ...
                    $attribute->product_id = $Did; // $id is passed in up there to the addAttributes() method
                    $attribute->sku = $value;
    ...
    { // $id is the URL Paramter (slug) passed from the URL
        Session::put('page', 'products');
        $Did = $id ? decrypt($id) : '';
        $product = Product::select('id', 'product_name', 'product_code', 'product_color', 'product_price', 'product_image')->with('images')->find($Did); // with('images') is the relationship method name in the Product.php model
