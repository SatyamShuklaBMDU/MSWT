...
class NewsletterController extends Controller
{
    ...
    public function subscribers(Request $request) {
        ...
        if($request->startt!="" && $request->endd!=""){
           $subscrib= NewsletterSubscriber::whereBetween('created_at',[$request->startt,$request->endd])->get();
           $subscribers=[];
           if(count($subscrib)>0){
               foreach($subscrib as $subscribe){
                    $subscribers[]=User::where('email',$subscribe->email)->first();
               }
            }
       }else{
        $subscribers = NewsletterSubscriber::join('users','users.email','newsletter_subscribers.email')->select('users.*','newsletter_subscribers.*')->get()->toArray();
    }
        // dd($subscribers);
    ...
    public function changeStatusSubscribers(Request $request)
    {
       $userId = $request->input('userId');
        $status = $request->input('status');

        // Update the status of the user in the database
        $user = NewsletterSubscriber::find($userId);
        if ($user) {
            $user->status = $status;
            $user->save();
             $users = NewsletterSubscriber::find($userId);   
            return response()->json(['success' => true,'users'=>$users, 'message' => 'Status updated successfully']);
        } else {
