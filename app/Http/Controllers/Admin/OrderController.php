...
class OrderController extends Controller
{
    ...
    public function orderDetails($id)
    {
        ...
        if ($adminType == 'vendor') { // If the authenticated/logged-in user is 'vendor', we show ONLY the details of the orders of the products added by that specific 'vendor' ONLY (from `orders_products` table) in admin/orders/order_details.blade.php page
            ...
            ])->where('id', $Did)->first()->toArray(); // Eager Loading: https://laravel.com/docs/9.x/eloquent-relationships#eager-loading    // 'orders_products' is the relationship method name in Order.php model
            // dd($orderDetails);
            ...
            $orderDetails = Order::with('orders_products')->where('id', $Did)->first()->toArray(); // Eager Loading: https://laravel.com/docs/9.x/eloquent-relationships#eager-loading    // 'orders_products' is the relationship method name in Order.php model
            // dd($orderDetails);
        }
        ...
        $orderLog = OrdersLog::with('orders_products')->where('order_id', $Did)->orderBy('id', 'Desc')->get()->toArray(); // Show Order Log descendingly    // Eager Loading: https://laravel.com/docs/9.x/eloquent-relationships#eager-loading    // 'orders_products' is the relationship method name in OrdersLog.php model
        // dd($orderLog);
    ...
    public function updateOrderItemStatus(Request $request)
    {
        if ($request->isMethod('post')) {
            $rules = [
                'order_item_status' => 'required',
            ];
            $messages = [
                'order_item_status.required' => 'The order status is required.',
            ];
