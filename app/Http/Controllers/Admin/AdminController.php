...
class AdminController extends Controller
{
    ...
    public function showallAdminPage()
    {
        Session::put('page', 'all_admin');
        $admins = Admin::where('type', '!=', 'superadmin')->get();
        return view('admin.admins.all_admins')->with(compact('admins'));
    }
    public function updateUserAdminStatus(Request $request)
    { // Update Product Status using AJAX in products.blade.php
        if ($request->ajax()) { // if the request is coming via an AJAX call
            ...
            Admin::where('id', $data['admin_id'])->update(['status' => $status]); // $data['product_id'] comes from the 'data' object inside the $.ajax() method
            // echo '<pre>', var_dump($data), '</pre>';
    ...
    public function editAdminPage(Request $request, $id)
    {
        if ($request->isMethod('get')) {
            $Did = $id ? decrypt($id) : "";
            $admin = Admin::where('id', $Did)->first();
            // dd($admin);
            return view('admin.admins.admins')->with(compact('admin'));
        } elseif ($request->isMethod('post')) {
            $Did = $id ? decrypt($id) : "";
            Admin::find($Did)->update([
                'name' => $request->name,
                'email' => $request->email,
                'type' => $request->role,
                'mobile' => $request->mobile,
            ...
            return redirect('/all-manage')->with('success_message', 'Admin has been updated successfully!');
        }
    }
}
